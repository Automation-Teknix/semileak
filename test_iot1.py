...
class IoTDevice:
    ...
    def accept_connection(self) -> bool:
        with self.lock:
            ...
            if self.connected:
                self.logger.debug("Already connected to client")
                return True
        ...
        with self.lock:
            ...
            if self.client_socket:
                try:
                    ...
                    self.logger.info("Client connection closed properly")
                except Exception as e:
        ...
        try:
            ...
            for di_name, di_val in di_transitions.items():
                ...
                if di_val == 1 and state['active']:
                    # Collect only the corresponding AI value for this DI
                    if ai_name in data:
                        ai_val = float(data[ai_name])
                        state['ai_window'].append((ai_name, ai_val, time.time()))
                    # First 5s check
                    if not state['first_check_done'] and time.time() >= state['first_check_time']:
                        window_vals = [x for x in state['ai_window'] if x[2] - state['start_time'] <= 5]
                        if window_vals:
                ...
                if k.startswith("AI"):
                    try:
                        ...
                        if result is not None and result > 0:
                            self.data_logger.debug(f"Successfully inserted AI {k}")
                            processed_count += 1
                            self.stats['successful_db_operations'] += 1
                        else:
                            self.data_logger.error(f"Insert failed for AI {k}")
                            self.stats['failed_db_operations'] += 1
