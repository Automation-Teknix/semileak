#!/usr/bin/env python3
"""
Gateway 1 IoT Data Processor - Handles AI1-AI6 and DI1-DI6
...
class OptimizedDatabaseConnection:
    def __init__(self, host: str, user: str, password: str, database: str, port: int = 3306):
        self.logger = logging.getLogger('gateway1.database')
        self.host = host
        self.user = user
    ...
    def connect(self):
        try:
            ...
            cursor.close()
            self.logger.info("Gateway 1 database connection established")
...
class Gateway1Processor:
    def __init__(self, db_config: dict):
        self.logger = logging.getLogger('gateway1.processor')
        self.db = OptimizedDatabaseConnection(**db_config)
        
        # Gateway 1 specific configuration
        self.device_id = "gateway_1"
        self.ai_range = list(range(1, 7))  # AI1-AI6
        self.di_range = list(range(1, 7))  # DI1-DI6
        ...
        # Event-driven DI/AI state tracking (only for Gateway 1 DIs)
        self.di_states = {}
    ...
    def load_gateway_config(self):
        """Load Gateway 1 specific configuration from database"""
        try:
            ...
            if result:
                self.ip = result[0]["iot_gateway1_ip"] or "0.0.0.0"
                self.port = int(result[0]["iot_gateway1_port"]) if result[0]["iot_gateway1_port"] else 8001
                self.logger.info(f"Gateway 1 configured: {self.ip}:{self.port}")
            else:
                self.port = 8001  # Default port
    ...
    def get_cached_calibration_values(self):
        ...
        if current_time - self.calibration_cache['last_update'] > self.calibration_cache['cache_duration']:
            try:
                # Get only AI1-AI6 calibration values
                c_query = "SELECT cValue1, cValue2, cValue3, cValue4, cValue5, cValue6 FROM c_values ORDER BY id DESC LIMIT 1"
                m_query = "SELECT mValue1, mValue2, mValue3, mValue4, mValue5, mValue6 FROM m_values ORDER BY id DESC LIMIT 1"
                ...
                # Process C values for AI1-AI6 only
                if c_result:
                ...
                # Process M values for AI1-AI6 only
                if m_result:
                    for i in self.ai_range:
                        col_name = f"mValue{i}"
    ...
    def setup_socket(self):
        """Setup socket server for Gateway 1"""
        try:
            self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            self.server_socket.bind((self.ip, self.port))
            self.server_socket.listen(1)
            self.logger.info(f"Gateway 1 server listening on {self.ip}:{self.port}")
        except Exception as e:
    ...
    def accept_connection(self):
        ...
        try:
            ...
            self.connected = True
            self.logger.info(f"Gateway 1 client connected from {addr}")
            return True
    ...
    def process_batch_data(self, data_batch):
        ...
        for data_str in data_batch:
            try:
                ...
                # Process AI values (AI1-AI6 only)
                for i in self.ai_range:
                    ai_key = f"AI{i}"
                    if ai_key in data:
                        try:
                ...
                # Process DI values and event logic (DI1-DI6 only)
                for i in self.di_range:
                    di_key = f"DI{i}"
                    ai_key = f"AI{i}"
    ...
    def update_connection_status(self, status):
        ...
        try:
            ...
            select_query = "SELECT server_connection_1 FROM myplclog ORDER BY id DESC LIMIT 1"
            ...
            if current_val != status:
                update_query = "UPDATE myplclog SET server_connection_1 = %s WHERE id = (SELECT * FROM (SELECT MAX(id) FROM myplclog) AS temp)"
    ...
    def log_stats(self):
        ...
        self.logger.info(f"Gateway 1 Stats - Messages: {self.stats['messages_processed']}, "
                        f"Records: {self.stats['records_inserted']}, Rate: {rate:.1f}/sec, "
                        f"DB Ops: {self.stats['db_operations']}, Errors: {self.stats['errors']}")

    def run(self):
        """Main processing loop"""
        self.logger.info("Starting Gateway 1 Processor")
            ...
            try:
                ...
                if data_list:
                    ...
                    if not self.connected:
                        ...
                        self.logger.warning("Gateway 1 disconnected")
                        time.sleep(1)
    ...
    def shutdown(self):
        """Clean shutdown"""
        self.logger.info("Shutting down Gateway 1 Processor")
        self.running = False
        ...
        self.log_stats()
        self.logger.info("Gateway 1 Processor shutdown complete")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'gateway1_{datetime.datetime.now().strftime("%Y%m%d")}.log'),
    ...
    processor = Gateway1Processor(db_config)
